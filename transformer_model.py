...
def train_multi_task_model(model_name='distilbert-base-uncased', epochs=3, batch_size=16, lr=2e-5):
    ...
    try:
        ...
        # Handle class imbalance using weighted cross-entropy
        imp_weights = compute_class_weight('balanced', classes=np.unique(y_imp_tr), y=y_imp_tr)
        lik_weights = compute_class_weight('balanced', classes=np.unique(y_lik_tr), y=y_lik_tr)
...
if __name__ == "__main__":
    # Can toggle to 'bert-base-uncased' if desired
